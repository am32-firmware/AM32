...
{ 
...
#ifdef CURRENT_ADC_PIN
	LL_GPIO_SetPinMode(GPIOA, CURRENT_ADC_PIN, LL_GPIO_MODE_ANALOG);
#endif

#ifdef VOLTAGE_ADC_PIN
	LL_GPIO_SetPinMode(GPIOA, VOLTAGE_ADC_PIN, LL_GPIO_MODE_ANALOG);
#endif
	...
	// LL_ADC_REG_SetTriggerEdge(ADC1, LL_ADC_REG_TRIG_EXT_RISING);
		...
		LL_ADC_REG_SetSequencerRanks(ADC1, LL_ADC_REG_RANK_3, ADC_CHANNEL_TEMP); //external temp sensor
#else
		LL_ADC_REG_SetSequencerRanks(ADC1, LL_ADC_REG_RANK_3, LL_ADC_CHANNEL_17); //temp sensor on mcu	
#endif
...
#endif
#ifdef ADC_CHANNEL_TEMP
		LL_ADC_SetChannelSamplingTime(ADC1, ADC_CHANNEL_TEMP, LL_ADC_SAMPLINGTIME_47CYCLES_5);
#else
