...
//	dma_interrupt_enable(INPUT_DMA_CHANNEL, DMA_INT_FTF);
  ...
  //LL_DMA_EnableChannel(DMA1, INPUT_DMA_CHANNEL);
	...
	// TMR_DMAConfig(IC_TIMER_REGISTER, TMR_DMABase_CC1, TMR_DMABurstLength_1Transfer);
...
//DMA_CHCTL(INPUT_DMA_CHANNEL) = 0x0000098b;   // just set the whole reg in one g to enable

//gpio_mode_QUICK(INPUT_PIN_PORT, GPIO_Mode_AF, GPIO_Pull_NOPULL, INPUT_PIN);
...
//	  LL_TIM_EnableCounter(IC_TIMER_REGISTER);
		...
		//		 gpio_mode_QUICK(GPIOB, GPIO_Mode_AF, GPIO_Pull_NOPULL, INPUT_PIN);
...
// 	if ((smallestnumber > 1)&&(smallestnumber <= 5)&& (average_signal_pulse < 60)) {
