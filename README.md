...
To use AM32 firmware on a blank ESC, a bootloader must first be installed using an ST-LINK, GD-LINK , CMIS-DAP or AT-LINK.  The bootloader will be depend on the MCU used on the ESC. Choose the bootloader that matches the MCU type and signal input pin of the ESC.
...
## Sponsors
The AM32 project would not have made it this far without help from the following sponsors:
