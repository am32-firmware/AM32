...
uint16_t duty_cycle_maximum =
    TIM1_AUTORELOAD; // restricted by temperature or low rpm throttle protect
...
void checkForHighSignal() {
  ...
  for (int i = 0; i < 1000; i++) {
    if (!(getInputPinState())) { // if the pin is low for 5 checks out of 100 in
                                 // 100ms or more its either no signal or
                                 // signal. jump to application
      low_pin_count++;
    }
    delayMicros(10);
  }
...
void loadEEpromSettings() {
  ...
  if (eepromBuffer[24] < 49 && eepromBuffer[24] > 7) {
    if (eepromBuffer[24] < 49 && eepromBuffer[24] > 23) {
      TIMER1_MAX_ARR =
          map(eepromBuffer[24], 24, 48, TIM1_AUTORELOAD, TIM1_AUTORELOAD / 2);
    }
    if (eepromBuffer[24] < 24 && eepromBuffer[24] > 11) {
      TIMER1_MAX_ARR =
          map(eepromBuffer[24], 12, 24, TIM1_AUTORELOAD * 2, TIM1_AUTORELOAD);
    }
  ...
  if (eepromBuffer[25] < 151 && eepromBuffer[25] > 49) {
    ...
    minimum_duty_cycle =
        (eepromBuffer[25] / 2 + DEAD_TIME / 3) * TIMER1_MAX_ARR / 2000;
  ...
  if (eepromBuffer[1] >
    ...
    servo_low_threshold =
        (eepromBuffer[32] * 2) + 750; // anything below this point considered 0
    ...
    low_cell_volt_cutoff =
        eepromBuffer[37] + 250; // 2.5 to 3.5 volts per cell range
    ...
    if (eepromBuffer[40] > 4 &&
        eepromBuffer[40] < 26) { // sine mode changeover 5-25 percent throttle
...
void commutate() {
  ...
  if (use_speed_control_loop && running) {
    input_override +=
        doPidCalculations(&speedPid, e_com_time, target_e_com_time) / 10000;
...
void PeriodElapsedCallback() {
  ...
  advance = (commutation_interval >> 3) *
            advance_level; // 60 divde 8 7.5 degree increments
...
void tenKhzRoutine() { // 20khz as of 2.00 to be renamed
  ...
  //	start_process = UTILITY_TIMER->CNT;
...
#ifndef BRUSHED_MODE
  if (bemf_timeout_happened > bemf_timeout * (1 + (crawler_mode * 100)) &&
    ...
    if (use_sin_start) {
      ...
      if (adjusted_input > 30 &&
          adjusted_input < (sine_mode_changeover_thottle_level * 20)) {
  ...
  if (!armed && (cell_count == 0)) {
    if (inputSet) {
      if (adjusted_input == 0) {
        armed_timeout_count++;
        if (armed_timeout_count > 20000) { // one second
  ...
  if (!stepper_sine) {
    if (input >= 47 + (80 * use_sin_start) && armed) {
      ...
      if (tenkhzcounter % 20 == 0) { // 1khz PID loop
        ...
        if (stall_protection &&
            running) { // this boosts throttle as the rpm gets lower, for
                       // crawlers and rc cars only, do not use for multirotors.
        ...
        //			  if(use_speed_control_loop && running){
        //			  input_override += doPidCalculations(&speedPid,
        // e_com_time, target_e_com_time)/10000;
  ...
  signaltimeout++;
  if (signaltimeout >
      10000) { // half second timeout when armed half second for servo;
    if (armed) {
      allOff();
      armed = 0;
      input = 0;
    ...
    if (signaltimeout > 50000) { // 2.5 second
      allOff();
      armed = 0;
      input = 0;
      inputSet = 0;
...
#endif
  //	process_time =  UTILITY_TIMER->CNT -  start_process;
}
...
#ifdef GIMBAL_MODE
  SET_PWM_COMPARE(1, (((2 * pwmSin[phase_A_position]) + gate_drive_offset) *
                      TIMER1_MAX_ARR / 2000));
  SET_PWM_COMPARE(2, (((2 * pwmSin[phase_B_position]) + gate_drive_offset) *
                      TIMER1_MAX_ARR / 2000));
  SET_PWM_COMPARE(3, (((2 * pwmSin[phase_C_position]) + gate_drive_offset) *
                      TIMER1_MAX_ARR / 2000));
#else
  SET_PWM_COMPARE(
      1, ((((2 * pwmSin[phase_A_position] / SINE_DIVIDER) + gate_drive_offset) *
           TIMER1_MAX_ARR / 2000) *
          sine_mode_power / 10));
  SET_PWM_COMPARE(
      2, ((((2 * pwmSin[phase_B_position] / SINE_DIVIDER) + gate_drive_offset) *
           TIMER1_MAX_ARR / 2000) *
          sine_mode_power / 10));
  SET_PWM_COMPARE(
      3, ((((2 * pwmSin[phase_C_position] / SINE_DIVIDER) + gate_drive_offset) *
           TIMER1_MAX_ARR / 2000) *
          sine_mode_power / 10));
#endif
}
...
int main(void) {
  ...
  if (firmware_info.version_major != eepromBuffer[3] ||
      firmware_info.version_minor != eepromBuffer[4]) {
  ...
  startup_max_duty_cycle = startup_max_duty_cycle * TIMER1_MAX_ARR / 2000 +
                           dead_time_override; // adjust for pwm frequency
  throttle_max_at_low_rpm = throttle_max_at_low_rpm * TIMER1_MAX_ARR /
                            2000;            // adjust to new pwm frequency
  ...
  while (1) {
    ...
    if (adc_counter > 10) { // for adc and telemetry
      ...
      smoothed_raw_current =
          ((63 * smoothed_raw_current + (ADC_raw_current)) >> 6);
      actual_current = ((smoothed_raw_current * 3300 / 41) - CURRENT_OFFSET) /
                       (MILLIVOLT_PER_AMP);
      if (actual_current < 0) {
        actual_current = 0;
      }
    ...
    //   			  if(RC_CAR_REVERSE){
    //   				  if (newinput > (1000 +
    //   (servo_dead_band<<1))) { 					  if
    //   (forward == dir_reversed) {
    //   adjusted_input = 0; if(running){
    //   prop_brake_active = 1; }else{
    //   forward = 1 - dir_reversed;
    //   						  }
    //   					  }
    //   					  if (prop_brake_active == 0) {
    ...
    //   				  if (newinput < (1000
    //   -(servo_dead_band<<1))) { 					  if
    //   (forward == (1 - dir_reversed)) { 						  if(running){ 							  prop_brake_active = 1;
    //   }else{ 							  forward =
    //   dir_reversed;
    //   						  }
    //   						  adjusted_input = 0;
    ...
    //   				  if (newinput >= (1000 -
    //   (servo_dead_band
    //   << 1)) && newinput <= (1000 + (servo_dead_band <<1))) {
    //   adjusted_input = 0; prop_brake_active = 0;
    //   				  }
    //   			  }else{
    //   				  if (newinput > (1000 +
    //   (servo_dead_band<<1))) { 					  if
    //   (forward == dir_reversed) { 						  if((commutation_interval >
    //   reverse_speed_threshold )|| stepper_sine){
    //   forward = 1 - dir_reversed; 							  zero_crosses = 0;
    //   old_routine = 1;
    //   maskPhaseInterrupts();
    //   brushed_direction_set = 0; 						  }else{ 							  newinput = 1000;
    //   						  }
    //   					  }
    //   					  adjusted_input = map(newinput,
    //   1000 + (servo_dead_band<<1), 2000, 47, 2047);
    //   				  }
    //   				  if (newinput < (1000
    //   -(servo_dead_band<<1))) { 					  if
    //   (forward == (1 - dir_reversed)) { 						  if((commutation_interval >
    //   reverse_speed_threshold) || stepper_sine){
    //   zero_crosses = 0; 							  old_routine = 1;
    //   forward = dir_reversed;
    //   maskPhaseInterrupts();
    //   brushed_direction_set
    //   = 0; 						  }else{
    //   newinput = 1000;
    //
    //   						  }
    //   					  }
    //   					  adjusted_input = map(newinput,
    //   0, 1000-(servo_dead_band<<1), 2047, 47);
    //   				  }
    //
    //  				  if (newinput >= (1000 -
    //  (servo_dead_band << 1)) && newinput <= (1000 + (servo_dead_band <<1))) {
    //   					  adjusted_input = 0;
    //   					brushed_direction_set = 0;
    //   				  }
    //   			  }
    ...
    //
    //   				  if (forward == dir_reversed) {
    //   					  if(commutation_interval >
    //   reverse_speed_threshold || stepper_sine){
    //   forward = 1 - dir_reversed; 						  zero_crosses = 0; 						  old_routine = 1;
    //   						  maskPhaseInterrupts();
    //   						brushed_direction_set =
    //   0; 					  }else{
    //   newinput = 0;
    ...
    //   					  }
    //   				  }
    //   				  adjusted_input = ((newinput - 1048) * 2
    //   + 47) - reversing_dead_band;
    //
    //   			  }
    //   			  if (newinput <= 1047  && newinput > 47) {
    //   				  //	startcount++;
    ...
    //   					  if(commutation_interval >
    //   reverse_speed_threshold || stepper_sine){
    //   zero_crosses = 0; old_routine
    //   = 1; 						  forward =
    //   dir_reversed; maskPhaseInterrupts(); 						brushed_direction_set = 0;
    //   }else{ 						  newinput = 0;
    //
    //   					  }
    //   				  }
    //   				  adjusted_input = ((newinput - 48) * 2
    //   + 47) - reversing_dead_band;
    //   			  }
    //   			  if ( newinput < 48) {
    //   				  adjusted_input = 0;
    ...
    //  bemf_timeout =
    //  100; 	 	 	 }else{ bemf_timeout = 10;
    //  	 	 	 }
    //  	 	 }
    //	  if(bemf_timeout_happened > bemf_timeout * ( 1 + (crawler_mode*100))&&
    // stuck_rotor_protection){ 	 		 allOff(); 	 		 maskPhaseInterrupts();
    // input = 0; 	 		bemf_timeout_happened = 102;
    // #ifdef USE_RGB_LED
    //			  GPIOB->BRR = LL_GPIO_PIN_8; // on red
    //			  GPIOB->BSRR = LL_GPIO_PIN_5;  //
    //			  GPIOB->BSRR = LL_GPIO_PIN_3;
    ...
    //   					if(adjusted_input > 30 &&
    //   adjusted_input < (sine_mode_changeover_thottle_level * 20)){
    //   input= map(adjusted_input, 30 , (sine_mode_changeover_thottle_level *
    //   20) , 47 ,160);
    //   					}
    //   					if(adjusted_input >=
    //   (sine_mode_changeover_thottle_level * 20)){
    //   input = map(adjusted_input , (sine_mode_changeover_thottle_level * 20)
    //   ,2047 , 160, 2047);
    //   					}
    //   				}else{
    ...
    //   					  if (drive_by_rpm){
    //  						target_e_com_time =
    //  60000000 / map(adjusted_input , 47 ,2047 , MINIMUM_RPM_SPEED_CONTROL,
    //  MAXIMUM_RPM_SPEED_CONTROL) / (motor_poles/2);
    //   		  				if(adjusted_input < 47){
    //   // dead band ? input= 0;
    //   speedPid.error = 0;
    //   input_override = 0; 		  				}else{
    //   	  						input =
    //   (uint16_t)input_override;  // speed control pid override
    //   	  						if(input_override
    //   >
    //   2047){ 	  							input
    //   = 2047;
    ...
    if (stepper_sine == 0) {
      ...
      if (degrees_celsius > TEMPERATURE_LIMIT) {
        duty_cycle_maximum =
            map(degrees_celsius, TEMPERATURE_LIMIT - 10, TEMPERATURE_LIMIT + 10,
      ...
      if (input > 48 && armed) {
        ...
        if (input > 48 && input < 137) { // sine wave stepper
          ...
          advanceincrement();
          step_delay =
              map(input, 48, 120, 7000 / motor_poles, 810 / motor_poles);
          delayMicros(step_delay);
          e_rpm =
