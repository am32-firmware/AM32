...
void ADC_Init(void)
{
    ...
    DDL_ADC_SetChannelSamplingTime(ADC, VOLTAGE_ADC_CHANNEL, DDL_ADC_SAMPLINGTIME_8_SCYCLES);
    DDL_ADC_SetChannelSamplingTime(ADC, CURRENT_ADC_CHANNEL, DDL_ADC_SAMPLINGTIME_8_SCYCLES);
    DDL_ADC_SetChannelSamplingTime(ADC, TSENSOR_ADC_CHANNEL, DDL_ADC_SAMPLINGTIME_32_SCYCLES);
    DDL_ADC_SetChannelSamplingTime(ADC, INPUT_ADC_CHANNEL, DDL_ADC_SAMPLINGTIME_8_SCYCLES);
    ...
    DDL_ADC_REG_SetSequencerRanks(ADC, DDL_ADC_REG_RANK_3, TSENSOR_ADC_CHANNEL);
#ifdef USE_ADC_INPUT
    DDL_ADC_REG_SetSequencerRanks(ADC, DDL_ADC_REG_RANK_4, INPUT_ADC_CHANNEL);
    DDL_ADC_SEQ_SetSectionTimes(ADC, DDL_ADC_SEQ_SECTION_1, DDL_ADC_SEQ_TRS_TIME_4);
#else
...
{
    return -(((int32_t)adcrawtemp*8 - 13656)*841>>15) + 25; // 1/39==841>>15
}
