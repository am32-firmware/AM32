...
class MspPort(object):
    ...
    def __init__(self, port, baud=115200, callbacks=None):
        ...
        # a wedged FC stops draining its USB endpoint, and without a
        # write timeout pyserial then blocks here for ever
    ...
    def send(self, cmd, payload=b''):
        ...
        self._write(frame(cmd, payload))

    def _write(self, data):
        try:
            self.ser.write(data)
        except serial.SerialTimeoutException:
            raise IOError('the flight controller stopped accepting data on '
                          '%s - unplug and replug it' % self.ser.port)
...
BATTERY_STATES = {0: 'ok', 1: 'warning', 2: 'critical',
                  3: 'not present', 4: 'init'}


def parse_battery_state(data):
    """cell count, pack voltage and whether the FC sees a battery at all"""
    if len(data) < 9:
        return {}
    cells = data[0]
    volts = data[3] / 10.0
    state = data[8]
    if len(data) >= 11:                 # 0.01V field on newer APIs
        volts = struct.unpack('<H', data[9:11])[0] / 100.0
    return {'cells': cells, 'volts': volts, 'state': state,
            'state_name': BATTERY_STATES.get(state, 'unknown'),
            'present': state != 3}
