...
void interrupts_init(void)
{
    ...
    /* Used for delaying interrupt commutation */
    ...
    /* Used for pwm input capture */
    NVIC_EnableIRQ(DMA_CH1_IRQn);
    NVIC_SetPriority(DMA_CH1_IRQn, 2);

    /* Used for fast response sending and receiving with the Dshot protocol */
    NVIC_EnableIRQ(EINT0_1_IRQn);
    NVIC_SetPriority(EINT0_1_IRQn, 3);

    /* Used for quick loop frequency timed interrupt */
...
{
    DDL_GPIO_InitTypeDef        GPIO_InitStructure = {0};
    DDL_ATMR_InitTypeDef        TIM_TimeBaseInitStructure = {0};
    DDL_ATMR_BDT_InitTypeDef    TIM_BDTRInitStructure = {0};
    DDL_ATMR_OC_InitTypeDef     TIM_OCInitStructure = {0};
    ...
    GPIO_InitStructure.Alternate  =  DDL_GPIO_AF_3;
...
void drv_btmr0_init(void)
{
    ...
    DDL_BTMR_EnableARRPreload(BTMR0);
    DDL_BTMR_EnablePSCPreload(BTMR0);
}
...
void drv_btmr1_init(void)
{
    ...
    DDL_BTMR_EnableARRPreload(BTMR1);
    DDL_BTMR_EnablePSCPreload(BTMR1);
}

void __attribute__((unused)) drv_comp0_init(void)
{
    DDL_GPIO_InitTypeDef    GPIO_InitStruct = {0};
    DDL_COMP0_InitTypeDef   COMP0_InitStruct = {0};
...
{
    DDL_GPIO_InitTypeDef    GPIO_InitStruct = {0};
    DDL_COMP1_InitTypeDef   COMP1_InitStruct = {0};
...
void drv_gtmr_init(void)
{
    ...
    DDL_GPIO_LockKey(INPUT_PIN_PORT, DDL_GPIO_LOCK_DISABLE);
    ...
    DDL_GPIO_Init(INPUT_PIN_PORT, &GPIO_InitStruct);
    
    DDL_GPIO_LockKey(INPUT_PIN_PORT, DDL_GPIO_LOCK_ENABLE);

    DMA_InitStruct.Peripheral              = DDL_DMA_PERIPHERAL_4;
    DMA_InitStruct.Direction               = DDL_DMA_DIRECTION_PERIPH_TO_MEMORY;
    DMA_InitStruct.NbData                  = sizeof(last_dma_buffer)/sizeof(last_dma_buffer[0]);
